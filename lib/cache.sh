...
function cache::restore() {
	...
	cache_restore_start_time=$(metadata::current_unix_time_ms)

	if [[ ! -d "${cache_dir}/.heroku/python" ]]; then
		metadata::set_string "cache_status" "empty"
		metadata::set_duration "cache_restore_duration" "${cache_restore_start_time}"
		return 0
	fi
	...
	cached_package_manager="$(metadata::get_previous "package_manager")"
	if [[ -z "${cached_package_manager}" ]]; then
		...
		case "${package_manager}" in
			pip)
				...
				cached_pip_version="$(metadata::get_previous "pip_version")"
			...
			pipenv)
				...
				cached_pipenv_version="$(metadata::get_previous "pipenv_version")"
			...
			poetry)
				...
				cached_poetry_version="$(metadata::get_previous "poetry_version")"
			...
			uv)
				...
				cached_uv_version="$(metadata::get_previous "uv_version")"
	...
	if [[ -n "${cache_invalidation_reasons[*]}" ]]; then
		...
		metadata::set_string "cache_status" "discarded"
	else
		...
		metadata::set_string "cache_status" "reused"
	fi
...
function cache::save() {
	...
	cache_save_start_time=$(metadata::current_unix_time_ms)
	...
	metadata::set_duration "cache_save_duration" "${cache_save_start_time}"
}
