...
function poetry::install_poetry() {
	...
	metadata::set_string "poetry_version" "${POETRY_VERSION}"
	...
	if [[ -e "${poetry_bin_dir}/poetry" ]]; then
		...
		if ! python -m venv --without-pip "${poetry_venv_dir}" |& output::indent; then
			...
			EOF
			metadata::set_string "failure_reason" "create-venv::poetry"
			exit 1
		fi
		...
		if ! {
			...
			EOF
			metadata::set_string "failure_reason" "install-package-manager::poetry"
			exit 1
		fi
...
function poetry::install_dependencies() {
	...
	if ! {
		...
		output::error <<-EOF
			...
			See the log output above for more information.
		EOF
		metadata::set_string "failure_reason" "install-dependencies::poetry"
		exit 1
	fi
}
