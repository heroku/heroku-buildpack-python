...
function poetry::install_poetry() {
	...
	# TODO: Move to where cache invalidation happens?
	...
	# Note: Whilst the Codon cache location remains stable from build to build, for Heroku CI,
	# the cache directory currently does not, so the cached Poetry can never be used there.
	if [[ -e "${poetry_bin_dir}/poetry" ]]; then
		puts-step "Using cached Poetry ${POETRY_VERSION}"
	else
...
function poetry::install_dependencies() {
	...
	puts-step "Installing dependencies using '${poetry_install_command[*]}'"
