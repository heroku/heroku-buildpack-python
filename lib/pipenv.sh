...
function pipenv::install_pipenv() {
	...
	metadata::set_string "pipenv_version" "${PIPENV_VERSION}"
	...
	if [[ -f "${pipenv_bin_dir}/pipenv" ]]; then
		...
		if ! python -m venv --without-pip "${pipenv_venv_dir}" |& output::indent; then
			...
			EOF
			metadata::set_string "failure_reason" "create-venv::pipenv"
			exit 1
		fi
		...
		if ! {
			...
			EOF
			metadata::set_string "failure_reason" "install-package-manager::pipenv"
			exit 1
		fi
...
function pipenv::install_dependencies() {
	...
	if ! {
		...
		output::error <<-EOF
			...
			See the log output above for more information.
		EOF
		metadata::set_string "failure_reason" "install-dependencies::pipenv"
		exit 1
	fi
}
