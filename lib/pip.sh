...
function pip::install_pip_setuptools_wheel() {
	...
	metadata::set_string "pip_version" "${PIP_VERSION}"
	...
	if [[ "${python_major_version}" == +(3.9|3.10|3.11|3.12) ]]; then
		metadata::set_string "setuptools_version" "${SETUPTOOLS_VERSION}"
		metadata::set_string "wheel_version" "${WHEEL_VERSION}"
		packages_to_install+=(
	...
	if ! {
		...
		EOF
		metadata::set_string "failure_reason" "install-package-manager::pip"
		exit 1
	fi
}
