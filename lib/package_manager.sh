...
function package_manager::determine_package_manager() {
	...
	if [[ -f "${build_dir}/Pipfile.lock" ]]; then
		...
		EOF
		metadata::set_string "failure_reason" "package-manager::pipenv-missing-lockfile"
		exit 1
	fi
	...
	if ((${#package_managers_found[@]} == 0)) && [[ -f "${build_dir}/setup.py" ]]; then
		...
		metadata::set_raw "setup_py_only" "true"
	else
		metadata::set_raw "setup_py_only" "false"
	fi
	...
	case "${num_package_managers_found}" in
		...
		0)
			...
			EOF
			metadata::set_string "failure_reason" "package-manager::none-found"
			exit 1
			;;
		*)
			...
			metadata::set_string "package_manager_multiple_found" "$(
				IFS=,
